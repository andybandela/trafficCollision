...
# Set data file path
filename = '/Users/andybandela/Desktop/Centennial/Winter 2024/Comp313 - Software Development Project 2/KSI.csv'
# path = ''
...
# Replace 'Yes' with 1 and fill missing values with 0 for specified columns, explicitly ensuring integer type
for column in columns_to_replace:
    df[column] = df[column].replace({'Yes': 1, np.nan: 0}).astype(int)
    df[column] = df[column].infer_objects().astype(int)
...
# Before parsing the 'DATE' column, let's remove timezone information if present
df['DATE'] = df['DATE'].str.replace(r'\+\d{2}$', '', regex=True)

# Now, parse the 'DATE' column without timezone information
...
with open("results.txt", "w") as results_file:
    for classifier_name, classifier in classifiers.items():
        ...
        # joblib.dump(best_estimator,  f'/{classifier_name.lower().replace(" ", "_")}_best_estimator.pkl')
        model_path = os.path.join(path, f'{classifier_name.lower().replace(" ", "_")}_best_estimator.pkl')
